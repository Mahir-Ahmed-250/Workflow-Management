...
export default function MemberResponsibility({ currentUser }: MemberResponsibilityProps) {
  ...
  const loadData = async (background = true) => {
    if (!background) {
      const cachedUsers = sessionStorage.getItem("wms_cache_users_active");
      const cachedHistory = sessionStorage.getItem("wms_cache_history");
      const cachedResp = sessionStorage.getItem(`wms_cache_resp_${selectedMonth}`);
    ...
    try {
      ...
      sessionStorage.setItem("wms_cache_users_active", JSON.stringify(activeUsers));
      ...
      sessionStorage.setItem("wms_cache_history", JSON.stringify(historyList));
      ...
      if (respData.success && respData.data) {
        ...
        sessionStorage.setItem(`wms_cache_resp_${selectedMonth}`, JSON.stringify(respMap));
      }
    } catch (err) {
